...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Aggregate counting records by image pair to identify easy or low-quality samples."
    )
    parser.add_argument(
        "--input-root",
        type=Path,
        ...
        help="One or more run directories or roots containing run directories.",
    )
    parser.add_argument(
        "--output",
        type=Path,
...
def item_key(record: Dict[str, Any]) -> Tuple[str, str, str, str, str, str]:
    subset = str(record_field_value(record, "subset"))
    subcategory = str(record_field_value(record, "subcategory") or record_field_value(record, "key"))
    edit_type = str(record_field_value(record, "edit_type"))
    ...
    return subset, subcategory, edit_type, source_variant, original_image_path, edited_image_path
...
def build_item_row(
    *,
    ...
    subset, subcategory, edit_type, source_variant, original_path, edited_path = key
...
def print_top_rows(*, title: str, rows: Sequence[Dict[str, Any]], top_k: int) -> None:
    print()
    print(title)
    print("Action           CF_N  CF_Acc  CF_Bias  Easy  Subset/Subcategory/Edit/Source  Edited_Image")
    for row in rows[:top_k]:
        print(
            ...
            f"{row['Subset']}/{row['Subcategory']}/{row['Edit_Type']}/{row['Source_Variant']:<8} "
            f"{row['Edited_Image']}"
        )
